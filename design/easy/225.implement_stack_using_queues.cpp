...
pop, and empty). Implement the MyStack class: void push(int x) Pushes element x
to the top of the stack. int pop() Removes the element on the top of the stack
and returns it. int top() Returns the element on the top of the stack. boolean
empty() Returns true if the stack is empty, false otherwise. Notes: You must use
...
language, the queue may not be supported natively. You may simulate a queue
...
class MyStack {
  ...
  public:
    ...
    int top() const { return customStack.front(); }

    bool empty() const { return customStack.empty(); }
};
